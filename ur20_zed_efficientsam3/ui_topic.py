...
from ur20_zed_interfaces.msg import ImagePrompts
...
class UserInput(Node): 
    def __init__(self):
        ...
        self.prompt_publisher_ = self.create_publisher(ImagePrompts, "/user_prompts", 10)
    ...
    def publish_prompts(self):
        self.get_logger().info("Publishing prompts...")
        msg = ImagePrompts()
        msg.user_image = self.bridge_.cv2_to_imgmsg(self.captured_frame_, encoding="bgr8")
        ...
        pt_neg.z = 0.0
        msg.prompts = [pt_pos, pt_neg]

        self.prompt_publisher_.publish(msg)
        cv2.waitKey(500)
        self.toggle_control_mode()      
